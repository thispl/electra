...
def get_rendered_template(doc, name=None, print_format=None, meta=None,
    no_letterhead=None, letterhead=None,trigger_print=False,settings=None):
    ...
    not_indoc = ["Batch"]
    header=footer=None
    if (doc.doctype not in not_indoc) and no_letterhead==0:
        header=frappe.utils.jinja.render_template(letter_head.content, {"doc": vars(doc)})
