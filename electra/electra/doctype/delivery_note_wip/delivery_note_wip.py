...
class DeliveryNoteWIP(Document):
	...
	if TYPE_CHECKING:
		...
		enquiry_reference_no: DF.Data | None
		...
		sales_person_designation: DF.ReadOnly | None
		sales_person_mobile: DF.ReadOnly | None
		sales_person_user: DF.Link | None
		sales_person_username: DF.ReadOnly | None
	...
	def __init__(self, *args, **kwargs):
		super(DeliveryNoteWIP, self).__init__(*args, **kwargs)
		self.status_updater = [
			{
				...
				[
					{
						...
						"second_join_field": "so_detail",
						"extra_cond": """ and exists (select name from `tabDelivery Note WIP`
					...
					},
					{
						...
						"target_field": "returned_qty",
						"target_parent_dt": "Delivery Note WIP",
	...
	def on_update_after_submit(self):
		...
		if self.is_return == 1:
			if self.return_against:
				frappe.db.set_value("Delivery Note WIP",self.return_against,"status","Return Issued")
				frappe.db.set_value("Delivery Note WIP",self.name,"status","Return")
		else:
			if self.per_billed < 100:
				
				frappe.db.set_value("Delivery Note WIP",self.name,"status","Submitted")
	...
	def on_cancel(self):
		frappe.db.set_value("Delivery Note WIP",self.name,"status","Cancelled")

	def validate(self):
		self.in_words = money_in_words(self.base_net_total, self.currency)
		self.validate_posting_time()
		self.validate_proj_cust()
		self.check_sales_order_on_hold_or_close()
		self.validate_warehouse()
		self.validate_return_qty()
	...
	def validate_return_qty(self):
		if self.is_return:
			for i in self.return_item:
				if i.qty>=0:
					frappe.throw(_("Return quantity for item {0} cannot be positive").format(i.item_code))
				else:
					dn_returns=frappe.get_all("Delivery Note WIP",{'return_against':self.return_against,'docstatus':('!=',2)},['name'])
					total_return_qty = -(i.qty)
					for dn in dn_returns:
	...
	def update_billing_status(self, update_modified=True):
		...
		for dn in set(updated_delivery_notes):
			dn_doc = self if (dn == self.name) else frappe.get_doc("Delivery Note WIP", dn)
...
def update_billed_amount_based_on_so(so_detail, update_modified=True):
	...
	dn = frappe.qb.DocType("Delivery Note WIP").as_("dn")
...
def get_returned_qty_map(delivery_note):
	...
	returned_qty_map = frappe._dict(
		...
		from `tabDelivery Note Item` dn_item, `tabDelivery Note WIP` dn
...
def make_sales_invoice(source_name, target_doc=None):
	doc = frappe.get_doc("Delivery Note WIP", source_name)
	...
	doc = get_mapped_doc(
		...
		source_name,
		{
			...
			"Delivery Note Item": {
				...
				"field_map": {
					"name": "custom_delivery_note_wip_item",
					"parent": "custom_delivery_note_wip",
					"so_detail": "so_detail",
...
def make_delivery_trip(source_name, target_doc=None):
	...
	doclist = get_mapped_doc(
		...
		source_name,
		{
			"Delivery Note WIP": {"doctype": "Delivery Trip", "validation": {"docstatus": ["=", 1]}},
			"Delivery Note Item": {
...
def make_installation_note(source_name, target_doc=None):
	...
	doclist = get_mapped_doc(
		...
		source_name,
		{
			"Delivery Note WIP": {"doctype": "Installation Note", "validation": {"docstatus": ["=", 1]}},
			"Delivery Note Item": {
...
def update_delivery_note_status(docname, status):
	dn = frappe.get_doc("Delivery Note WIP", docname)
	dn.update_status(status)


@frappe.whitelist()
def make_inter_company_purchase_receipt(source_name, target_doc=None):
	return make_inter_company_transaction("Delivery Note WIP", source_name, target_doc)
...
def make_inter_company_transaction(doctype, source_name, target_doc=None):
	...
	if doctype == "Delivery Note WIP":
		...
		source_doc = frappe.get_doc(doctype, source_name)
		target_doctype = "Delivery Note WIP"
