...
frappe.ui.form.on("Delivery Note WIP", {
	setup: function(frm) {
		...
		if (frm.doc.docstatus === 1 && frm.doc.is_return === 1 && frm.doc.per_billed !== 100) {
			frm.add_custom_button(__('Credit Note'), function() {
				frappe.model.open_mapped_doc({
					method: "electra.electra.doctype.delivery_note_wip.delivery_note_wip.make_sales_invoice",
					frm: cur_frm,
				})
			}, __('Create'));
...
erpnext.stock.DeliveryNoteController = class DeliveryNoteController extends erpnext.selling.SellingController {
	...
	refresh(doc, dt, dn) {
		...
		if (doc.docstatus > 0) {
			...
			cur_frm.add_custom_button(__("Stock Ledger"), function() {
				frappe.db.get_value('Stock Entry', {'custom_reference_type': "Delivery Note WIP", 'reference_number': me.frm.doc.name}, 'name', (r) => {
					// if (r && r.name) {
						frappe.route_options = {
							reference_no: me.frm.doc.name,
	...
	make_sales_return() {
		frappe.model.open_mapped_doc({
			method: "electra.electra.doctype.delivery_note_wip.delivery_note_wip.make_sales_return",
			frm: this.frm
		})
	}
	...
	update_status(status) {
		...
		frappe.call({
			method:"electra.electra.doctype.delivery_note_wip.delivery_note_wip.update_delivery_note_status",
...
// 	var dn_fields = frappe.meta.docfield_map['Delivery Note WIP'];
