...
import time
class ProjectDayPlanTimesheet(Document):
    def on_submit(self):
        for act in self.project_day_plan_employee:
            if act.activity:
                task_id = frappe.get_doc("Task",act.activity)
                if task_id.qty > 0:
                    task_id.pending_qty -= act.qty
                    task_id.completed_qty += act.qty
        ...
        for pro in self.project_day_plan_employee:
            if pro.total_working_hours:
                ...
                date = datetime.strptime(str(self.plan_date), '%Y-%m-%d').date()
                in_time = datetime.combine(date, in_t)
                frappe.errprint(in_time)
                ou_t = datetime.strptime(str(pro.to_time),'%H:%M:%S').time()
                out_time = datetime.combine(date, ou_t)
                ...
                if not get_att:
                    if proj_time >= eight_hours:
                        ...
                        att.ot = pro.ot
                        att.total_working_hours = pro.total_working_hours
