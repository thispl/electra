...
frappe.ui.form.on("Budgeted vs Actual Test Report", {
	refresh(frm) {
        // frm.set_df_property('print', "hidden", 1);
        if (!frm.is_new()) {
            frm.add_custom_button(__('Calculate'), function () {
                frappe.call({
                    method: 'electra.electra.doctype.budgeted_vs_actual_test_report.budgeted_vs_actual_test_report.update_project_values',
                    args: {
        ...
        if (!frm.setup_progress_subscription) {
            ...
            frappe.realtime.on('project_update_progress', function (data) {
                ...
                if (data.progress === 100) {
                    frappe.msgprint(__('Project calculation is completed !'));
                }
            });
        }
    },
