...
class ConsolidatedReceivable(Document):
	def on_submit(self):
		for i in self.order_invoice_list:
			...
			acc = frappe.db.get_list("Account",{"company":i.company,"account_number":"1310"},["name","account_currency"])
			for ac in acc:
				pay.paid_from = ac.name
				pay.paid_from_account_currency = ac.account_currency
			pay_acc = frappe.db.get_list("Account",{"company":i.company,"account_number":"1211"},["name","account_currency"])
