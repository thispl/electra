...
class BudgetedvsActualReport(Document):
    pass

@frappe.whitelist()    
def enqueue_create_project(name):
	frappe.enqueue(
		update_project_values, # python function or a module path as string
...
def update_project_values(name):
    ...
    sales_in = frappe.db.sql(
        ...
        (name,),
        as_dict=True)[0]
    sales_in_return = frappe.db.sql(
        """ SELECT SUM(total - discount_amount) as total FROM `tabSales Invoice` WHERE project = %s and docstatus = 1 and is_return = 1""",
        (name,),
        as_dict=True)[0]
    if not sales_in["total"]:
        sales_in["total"] = 0
    if not sales_in_return["total"]:
        sales_in_return["total"] = 0
    if sales_in_return["total"]:
        sales_in["total"] = sales_in["total"] + sales_in_return["total"]
...
def process_manpower(category, project_doc, project_budget, project_name):
    ...
    timesheet = frappe.db.sql("""
        SELECT e.designation, SUM(c.costing_amount) AS cost
        FROM `tabTimesheet` p
    ...
    # Prepare dict: {"Supervisor": xx, "Labour": yy}
    designation_costs = {row.designation: round(row.cost or 0, 2) for row in timesheet}

    supervisor_cost = designation_costs.get("Supervisor", 0)
    total_project_cost = sum(designation_costs.values())
    labour_cost = total_project_cost - supervisor_cost

    # While appending into project_doc
    for item in category["table"]:
        if item.worker == "Supervisor":
        ...
        total_op_cost = total_project_cost
...
@frappe.whitelist()
def actual_vs_budgeted_test(name):
...
'''
        ...
        for s in project.custom_accessories:
            ...
            data_row_color = "#f8b878" if serial_no_a % 2 == 0 else "#ffe5b4"
            data += '''<tr style="background-color:{};">'''.format(data_row_color)
            data += '''<td class="white-border" style="text-align: right;">%s</td><td class="white-border" colspan=1>%s</td><td class="white-border" colspan=1>%s</td><td class="white-border" colspan=1>%s</td><td class="white-border" colspan=1>%s</td>
                        <td class="white-border" colspan=1 style="text-align: right;">%s</td><td class="white-border" style="text-align:right" colspan=1>%s</td>
                        <td class="white-border" style="text-align:right" colspan=1>%s</td><td class="white-border" style="text-align:right" colspan=1>%s</td>
                        ...
                        serial_no_a, s.budget_code, s.part_number, s.description, s.unit, 
                        fmt_money(round(so_qty(project.budgeting, s.part_number, s.docname),2),2), fmt_money(round(s.qty, 2),2), fmt_money(round(s.rate, 2), 2),
                        fmt_money(round(s.amount, 2), 2), fmt_money(round(s.del_qty, 2),2), del_rate, fmt_money(round(s.del_amount, 2), 2),
                        fmt_money(round(so_qty(project.budgeting, s.part_number, s.docname) - s.del_qty, 2), 2), 
                        color, display_value
    ...
    if project.custom_subcontract:
        ...
        for s in project.custom_subcontract:
            ...
            data_row_color = "#f8b878" if serial_no_sb % 2 == 0 else "#ffe5b4"
            data += '''<tr style="background-color:{};">'''.format(data_row_color)
            data += '''<td class="white-border" style="text-align: right;">%s</td><td class="white-border" colspan=1>%s</td><td class="white-border" colspan=1>%s</td><td class="white-border" colspan=1>%s</td><td class="white-border" colspan=1>%s</td>
                        <td class="white-border" colspan=1 style="text-align: right;">%s</td><td class="white-border" style="text-align:right" colspan=1>%s</td>
                        <td class="white-border" style="text-align:right" colspan=1>%s</td><td class="white-border" style="text-align:right" colspan=1>%s</td>
                        ...
                        serial_no_sb, s.budget_code, s.part_number, s.description, s.unit, 
                        fmt_money(round(so_qty(project.budgeting, s.part_number, s.docname),2),2), fmt_money(round(s.qty, 2),2), fmt_money(round(s.rate, 2), 2),
                        fmt_money(round(s.amount, 2), 2), fmt_money(round(s.del_qty, 2),2), del_rate, fmt_money(round(s.del_amount, 2), 2),
                        fmt_money(round(so_qty(project.budgeting, s.part_number, s.docname) - s.del_qty, 2), 2), 
                        color, display_value
    ...
    if project.custom_design:
        ...
        for s in project.custom_design:
            ...
            data_row_color = "#f8b878" if serial_no_d % 2 == 0 else "#ffe5b4"
            data += '''<tr style="background-color:{};">'''.format(data_row_color)
            data += '''<td class="white-border" style="text-align: right;">%s</td><td class="white-border" colspan=1>%s</td><td class="white-border" colspan=1>%s</td><td class="white-border" colspan=1>%s</td><td class="white-border" colspan=1>%s</td>
                        <td class="white-border" colspan=1 style="text-align: right;">%s</td><td class="white-border" style="text-align:right" colspan=1>%s</td>
                        <td class="white-border" style="text-align:right" colspan=1>%s</td><td class="white-border" style="text-align:right" colspan=1>%s</td>
                        ...
                        serial_no_d, s.budget_code, s.part_number, s.description, s.unit, 
                        fmt_money(round(so_qty(project.budgeting, s.part_number, s.docname),2),2), fmt_money(round(s.qty, 2),2), fmt_money(round(s.rate, 2), 2),
                        fmt_money(round(s.amount, 2), 2), fmt_money(round(s.del_qty, 2),2), del_rate, fmt_money(round(s.del_amount, 2), 2),
                        fmt_money(round(so_qty(project.budgeting, s.part_number, s.docname) - s.del_qty, 2), 2), 
                        color, display_value
    ...
    if project.custom_manpower:
        ...
        data += f'''<tr style="font-weight: bold; background-color: #e35310;">
                ...
                <tr>'''
        data += '</table>'
        data += get_timesheet_project(name, project_materials_sno)
        project_materials_sno += 1
    ...
    data += '<div class="page-break"></div>'
    data += other_report(name, project_materials_sno)
    return data


@frappe.whitelist()
def other_report(project, project_materials_sno):
    # Prepare the initial HTML table structure with styles

    pro = frappe.get_doc("Project",project)
    other_items = frappe.db.sql("""
        SELECT pb.name AS budget_name, pb.voucher_type, pbi.account, pbi.debit_in_account_currency
        FROM `tabJournal Entry` pb
        INNER JOIN `tabJournal Entry Account` pbi ON pbi.parent = pb.name
        WHERE pbi.project = %s
        AND pb.docstatus = 1
    """, (project,), as_dict=True)
    ...
    if  other_items:
        ...
        <h4>{project_materials_sno}. Petty Cash</h4>'
        ...
        for item in other_items:
            ...
            amount = item['debit_in_account_currency'] if item['debit_in_account_currency'] else 0
# 			# Accumulate total amount
# 			total_amount += amount
            total_amount += amount
            ...
            table += f'''
                <tr style="background-color: {jl_color};">
                    ...
                    <td style="" class="white-border">{project}</td>
                    <td style="" class="white-border">{item.budget_name}</td> 
                    <td style="" class="white-border">{item.voucher_type}</td>
                    <td style="" class="white-border">{item.account}</td>
                    <td style="text-align: right;" class="white-border">{fmt_money(round(amount, 2),2)}</td> 
                </tr>'''
            serial_no += 1
...
@frappe.whitelist()
def get_timesheet_project(name, sow_sno):
    ...
    data = f'<h4>{sow_sno}.1. Timesheet Details (Actual)</h4>'
    data += "<table class='table table-bordered' style='margin-top: -5px;'>"
    data += """<tr>
...
def format_value(val):
    ...
    return '<td style="text-align:right; color:{};">{}</td>'.format(color, display_value)


@frappe.whitelist()
def actual_vs_budgeted_test_manuel():
    ...
    data += '<h3>Overall Summary</h3>'
    data += '<table style="width:100%; border-collapse:collapse;">'
    data += '<tr style="background-color:#f8b878; text-align:center"><td class="orange" style="border-left-style:hidden;">S.NO</td><td class="orange">Items</td><td class="orange">Budgeted</td><td class="orange">Actual</td><td class="orange" style="border-right-style:hidden;">Variance</td></tr>'
    ...
    data += '<td class="white-border" style="text-align:left">Petty Cash</td>'
    data += '<td class="white-border">-</td>'
    data += '<td class="white-border">%s</td>' % fmt_money(total_jl_cost(name), 2)
    data += '<td class="white-border">-</td>'
    data += '</tr>'
