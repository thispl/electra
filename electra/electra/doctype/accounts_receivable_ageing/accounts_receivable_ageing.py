...
def make_xlsx(data, sheet_name=None, wb=None, column_widths=None):
    ...
    if company:
        ...
        ws.append([args.company," "," "," "," "," "," ","","","",""]) 
        ws.append([""," "," "," "," "," "," ","","","",""])
    ws.append(["Accounts Receivable Ageing"," "," "," "," "," "," "," "])
    date = datetime.now()
    header2=["Report Date: "+date.strftime("%d-%m-%Y")]
    ws.append(header2 + [""] * 7)
    customer=frappe.db.get_value('Accounts Receivable Ageing',{"name":args.name},['customer'])
    ...
    for header in ws.iter_rows(min_row=1 , max_row=1, min_col=1, max_col=13):
        for cell in header:
            cell.font = Font(bold=True)
            cell.alignment = align_center
            # cell.fill = PatternFill(fgColor='fcff42', fill_type = "solid")
    for header in ws.iter_rows(min_row=2 , max_row=5, min_col=1, max_col=13):
        for cell in header:
            cell.font = Font(bold=True)
            cell.alignment = align_center
    for header in ws.iter_rows(min_row=7 , max_row=7, min_col=1, max_col=13):
        for cell in header:
            cell.font = Font(bold=True)
            cell.fill = PatternFill(fgColor='D3D3D3', fill_type = "solid")
            cell.alignment = align_center
    for header in ws.iter_rows(min_row=8 , max_row=len(accounts_aging(args)) +302 , min_col=1, max_col=13):
        for cell in header:
            cell.alignment = align_center
    for header in ws.iter_rows(min_row=len(accounts_aging(args)) + 7 , max_row=len(accounts_aging(args)) + 7 , min_col=1, max_col=13):
        for cell in header:
            cell.font = Font(bold=True)
    ...
    header_range = ws['A1':ws.cell(row=len(accounts_aging(args))+7, column=13).coordinate]
    ...
    ws.merge_cells(start_row=6, start_column=1, end_row=6, end_column=13 )
...
def accounts_aging(args):
    ...
    for entry in combined_data:
        data.append([
            ...
            entry['age'],entry['oustanding_0_30'],
            entry['oustanding_31_60'],
            entry['oustanding_61_90'],
            entry['oustanding_91_120'],
            entry['oustanding_above_121']
        ])
    data.append([
