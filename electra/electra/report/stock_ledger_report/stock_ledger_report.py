...
def execute(filters=None):
    ...
    for sle in sl_entries:
        ...
        to_company  = get_to_company(sle.voucher_type,sle.voucher_no)
        ...
        if opening_row:
            tot = opening_row["qty_after_transaction"] + (inqty + outqty)
        if include_uom:
...
def get_to_company(voucher_type,voucher_no):
    to_company = ''
    voucher_number = voucher_no
    if voucher_type == "Purchase Invoice":
        voucher_number = frappe.db.get_value("Purchase Invoice", {'name':voucher_no,'is_internal_supplier':1},[ "confirmation_number"])
...
def get_customer(voucher_type,voucher_no):
    ...
    if voucher_type == "Sales Invoice":
        customer = frappe.db.get_value("Sales Invoice",{'name':voucher_no},[ "customer"])

    elif voucher_type == "Delivery Note":
        customer = frappe.db.get_value("Delivery Note",{'name':voucher_no},[ "customer"])

    elif voucher_type == "Purchase Receipt":
        customer = frappe.db.get_value("Purchase Receipt",{'name':voucher_no},[ "supplier"])

    elif voucher_type == "Purchase Invoice":
        customer = frappe.db.get_value("Purchase Invoice",{'name':voucher_no},[ "supplier"])
    else:
        customer  = "NA"
    return customer
