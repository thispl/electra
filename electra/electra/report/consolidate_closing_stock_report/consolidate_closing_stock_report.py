...
# 		opening_val = entry['opening_val']
# 		bal_val = entry['bal_val']

# 		if company_name in aggregated_data:
# 			aggregated_data[company_name]['opening_val'] += opening_val
# 			aggregated_data[company_name]['bal_val'] += bal_val
# 		else:
# 			aggregated_data[company_name] = {'opening_val': opening_val, 'bal_val': bal_val}

# 	aggregated_data_list = [{'company': key, **values} for key, values in aggregated_data.items()]
# 	for entry in aggregated_data_list:
# 		data.append(entry)


# 	add_additional_uom_columns(columns, data, include_uom, conversion_factors)
# 	# frappe.errprint(add_additional_uom_columns)
# 	return columns, data
...
# 			# frappe.errprint(d.posting_date)
...
#             aggregated_data[company_name]['opening_val'] += opening_val
#             aggregated_data[company_name]['bal_val'] += bal_val
#             frappe.errprint(aggregated_data[company_name]['opening_val'])
#         else:
#             aggregated_data[company_name] = {'opening_val': opening_val, 'bal_val': bal_val}
...
#         gro = frappe.db.get_value("Company", {"name": company_name}, ["group"])
#         entry['group'] = gro
#         group_name = entry.get('group')
#         opening_val_group = entry.get('opening_val', 0)
#         bal_val_group = entry.get('bal_val', 0)
#         if group_name in group_wise_data:
#             group_wise_data[group_name]['opening_val_group'] += opening_val_group
#             group_wise_data[group_name]['bal_val_group'] += bal_val_group
#         else:
#             group_wise_data[group_name] = {'opening_val_group': opening_val_group, 'bal_val_group': bal_val_group}

#     sorted_aggregated_data_list = sorted(aggregated_data_list, key=lambda x: x['group'])
#     data = []
#     current_group = None

#     for entry in sorted_aggregated_data_list:
#         group_name = entry.get('group')
#         entry.update(group_wise_data.get(group_name, {}))
#         if current_group != group_name:
#             data.append({
#                 'group': group_name,
#                 'opening_val': group_wise_data[group_name]['opening_val_group'],
#                 'bal_val': group_wise_data[group_name]['bal_val_group']
#             })
...
def execute(filters: Optional[StockBalanceFilter] = None):
	...
	frappe.log_error("Consolidated", filters)
	bal = 0
	final = []
	is_reposting_item_valuation_in_progress()
	if not filters:
	...
	for entry in aggregated_data_list:
		...
		# Get group (set default if None)
		gro = frappe.db.get_value("Company", {"name": company_name}, "group") or "Ungrouped"
		entry['group'] = gro

		group_name = entry.get('group')
		opening_val_group = entry.get('opening_val', 0) or 0
		bal_val_group = entry.get('bal_val', 0) or 0

		if group_name in group_wise_data:
			group_wise_data[group_name]['opening_val_group'] += opening_val_group
			group_wise_data[group_name]['bal_val_group'] += bal_val_group
		else:
			group_wise_data[group_name] = {
				'opening_val_group': opening_val_group,
				'bal_val_group': bal_val_group
			}

	# Safe sorting (no None now)
	sorted_aggregated_data_list = sorted(
		aggregated_data_list,
		key=lambda x: x.get('group')
	)
	...
	for entry in sorted_aggregated_data_list:
		...
		if current_group != group_name:
			...
			current_group = group_name
		data.append(entry)    
	total_opening_val = sum(entry.get('opening_val', 0) for entry in data if entry.get('company'))
	total_closing_val = sum(entry.get('bal_val', 0) for entry in data if entry.get('company'))

	data.append({
		'group': 'Total',
		'opening_val': total_opening_val,
		'bal_val': total_closing_val,
	})
...
def apply_conditions(query, filters):
	...
	if to_date := filters.get("to_date"):
		query = query.where(sle.posting_date <= to_date).where(~sle.warehouse.like("Work In Progress%"))
	else:
		frappe.throw(_("'To Date' is required"))
	...
	if filters.get("warehouse"):
		...
		query = (
			...
			.on(warehouse_table.name == sle.warehouse)
			.where(warehouse_table.warehouse_type == warehouse_type)
			.where(~sle.warehouse.like("Work In Progress%"))
		)
