	...
	for group_by_key in iwb_map:
		...
		if item_map.get(item):
			...
			for record in data:
				...
				key = (company, item_code, item_name)
				
				if key not in consolidated_data:
					consolidated_data[key] = {'total_bal_qty': 0.0, 'total_bal_val': 0.0}
			...
			result = [
				...
				for (company, item_code, item_name), data in consolidated_data.items()
			]
...
def apply_conditions(query, filters):
	...
	if company := filters.get("company"):
		query = query.where(sle.company == company).where(~sle.warehouse.like("Work In Progress%"))
