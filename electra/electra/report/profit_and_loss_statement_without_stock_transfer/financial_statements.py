...
def apply_additional_conditions(doctype, query, from_date, ignore_closing_entries, filters):
	...
	cost_of_goods = frappe.db.get_value(
		"Account",
		{"account_name": "Cost of Goods Sold", "company": filters.company},
		["lft", "rgt"],
		as_dict=True,
	)

	if cost_of_goods:
		cost_of_goods_accounts = frappe.get_all(
			"Account",
			filters={
				"lft": [">=", cost_of_goods.lft],
				"rgt": ["<=", cost_of_goods.rgt],
				"company": filters.company,
			},
			pluck="name",
		)
