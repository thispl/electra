	...
	for i in purchase_invoice:
		...
		for j in gt.items[:1]:
			...
			if lc:
				...
				p = 0
			tot = round((i.base_grand_total+(p)),3)

			row = [i.posting_date,i.bill_no,i.name,i.supplier,i.currency,round(i.grand_total,3),round(i.base_grand_total,3),round((p),3),round(tot,3)]
			data.append(row)
	return data