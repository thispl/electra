...
def get_data(filters):
	...
	# )
	accounts = frappe.db.sql(
		"""select name, account_number, parent_account, account_name, root_type, report_type, lft, rgt from `tabAccount` where company in ("MARAZEEM SECURITY SERVICES","MARAZEEM SECURITY SERVICES - HO","MARAZEEM SECURITY SERVICES - SHOWROOM") order by lft """,as_dict=True)
	...
	# 	(filters.company,),
	# )[0]
	min_lft, max_rgt = frappe.db.sql("""select min(lft), max(rgt) from `tabAccount` where company in ("MARAZEEM SECURITY SERVICES","MARAZEEM SECURITY SERVICES - HO","MARAZEEM SECURITY SERVICES - SHOWROOM") """)[0]
	frappe.errprint(min_lft)
	frappe.errprint(max_rgt)
...
def get_rootwise_opening_balances(filters, report_type):
	...
	comp_list = ["MARAZEEM SECURITY SERVICES","MARAZEEM SECURITY SERVICES - HO","MARAZEEM SECURITY SERVICES - SHOWROOM"]
	for comp in comp_list:
		query_filters = {
			...
			"company_fb": frappe.db.get_value("Company", comp, "default_finance_book"),
		}
