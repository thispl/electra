...
def execute(filters=None):
	...
	for sle in sl_entries:
		...
		reference_details=get_stock_entry_details(sle.voucher_type,sle.voucher_no)
		...
		sle.update({"in_qty": max(sle.actual_qty, 0), "out_qty": min(sle.actual_qty, 0), 'reference_type':reference_details.get('reference_type'),'reference_no':reference_details.get('reference_no')})
...
def get_stock_ledger_entries(filters, items):
	...
	query = (
		...
		.where(
			...
			& (sle.warehouse.not_like('Work In Progress - %'))
		)
...
@frappe.whitelist()
def get_stock_entry_details(voucher_type,voucher_no):
	if voucher_type=='Stock Entry':
		stock_entry = frappe.get_doc("Stock Entry", voucher_no)
		if stock_entry:
			return {
				"reference_type": stock_entry.custom_reference_type,
				"reference_no": stock_entry.reference_number,
			}
		else:
			return {}
	else:
