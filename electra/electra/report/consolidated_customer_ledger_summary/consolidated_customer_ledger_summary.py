...
class PartyLedgerSummaryReport(object):
	...
	def prepare_conditions(self):
		...
		if self.filters.company:
			if self.filters.company == "Al - Shaghairi Trading and Contracting Company W.L.L (ELECTRA)":
				lft, rgt = frappe.get_cached_value('Company',self.filters.company,  ["lft", "rgt"])
				all_companies = frappe.db.sql_list("""select name from `tabCompany` where lft >= {0} and rgt <= {1} order by lft, rgt""".format(lft, rgt))
				
				query = "gle.company in %s" % ", ".join(["%s"]*len(all_companies)), tuple(all_companies)
				conditions.append("gle.company in ('Al - Shaghairi Trading and Contracting Company W.L.L (ELECTRA)', 'STEEL DIVISION - ELECTRA', 'MEP DIVISION - ELECTRA', 'TRADING DIVISION - ELECTRA', 'INTERIOR DIVISION - ELECTRA', 'ELECTRA - BINOMRAN SHOWROOM', 'ENGINEERING DIVISION - ELECTRA', 'ELECTRICAL DIVISION - ELECTRA', 'ELECTRA - BARWA SHOWROOM')")
			else:
