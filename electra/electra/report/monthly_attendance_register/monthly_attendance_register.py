...
def execute(filters=None):
	...
	if filters.group_by:
		emp_map, group_by_parameters = get_employee_details(filters,filters.group_by, filters.company)
		holiday_list = []
		for parameter in group_by_parameters:
			h_list = [emp_map[parameter][d]["holiday_list"] for d in emp_map[parameter] if emp_map[parameter][d]["holiday_list"]]
			holiday_list += h_list
	else:
		emp_map = get_employee_details(filters,filters.group_by, filters.company)
...
def get_employee_details(filters,group_by, company):
	emp_map = {}
	conditions = ''
	if filters.get("employee_grade"): conditions += " and grade = '%s' "%filters.employee_grade
	if filters.get("employment_type"): conditions += " and employment_type = '%s' "%filters.employment_type
	query = """select name, employee_name, designation, department, branch, company,
		holiday_list from `tabEmployee` where company = %s %s """ % (frappe.db.escape(company),conditions)
