frappe.pages['payment-reconciliation-manual'].on_page_load = function (wrapper) {
	let page = frappe.ui.make_app_page({
		parent: wrapper,
		title: __('Payment Reconciliation - User Manual'),
		single_column: true,
	});
	...
	$.get('/assets/electra/payment_reconciliation_manual.html', function (html) {
		let $html = $(html);
		...
		$content.html(styles + '<div class="pr-manual-wrapper">' + bodyContent + '</div>');
		...
		$content.html('<div class="alert alert-danger" style="margin:20px;">Could not load the manual. Please ensure the file exists at <code>/assets/electra/payment_reconciliation_manual.html</code>. Run <code>bench build</code> if needed.</div>');
	});
};
