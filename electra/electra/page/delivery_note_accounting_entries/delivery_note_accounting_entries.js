frappe.pages['delivery-note-accounting-entries'].on_page_load = function (wrapper) {
	let page = frappe.ui.make_app_page({
		parent: wrapper,
		title: __('Delivery Note - Accounting Entries'),
		single_column: true,
	});
	...
	$.get('/assets/electra/delivery_note_accounting_entries.html', function (html) {
		let $html = $(html);
		...
		$content.html(styles + '<div class="dn-manual-wrapper">' + bodyContent + '</div>');
		...
		$content.html('<div class="alert alert-danger" style="margin:20px;">Could not load the manual. Please ensure the file exists at <code>/assets/electra/delivery_note_accounting_entries.html</code>. Run <code>bench build</code> if needed.</div>');
	});
};
