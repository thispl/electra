frappe.pages['bank-reconciliation-manual'].on_page_load = function (wrapper) {
	...
	$.get('/assets/electra/bank_reconciliation_manual.html', function (html) {
		let $html = $(html);
		...
		$content.html(styles + '<div class="br-manual-wrapper">' + bodyContent + '</div>');
		...
		$content.html('<div class="alert alert-danger" style="margin:20px;">Could not load the manual. Please ensure the file exists at <code>/assets/electra/bank_reconciliation_manual.html</code>. Run <code>bench build</code> if needed.</div>');
	});
};
