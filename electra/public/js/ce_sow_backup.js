...
frappe.ui.form.on('CE SOW', {
	...
	validate: function (frm) {
		...
		var profit_amount = amount_with_overheads - amount
		var profit_percent = (amount / amount_with_overheads) * 100

		var table = `<table border='1' style="width:100%">
						<tr><th style="background-color:grey;color:white">Cost Amount</th><th  style="background-color:grey;color:white">Selling Amount</th><th  style="background-color:grey;color:white">Selling Amount with Overheads</th><th  style="background-color:grey;color:white">Profit Amount</th><th  style="background-color:grey;color:white">Profit %</th></tr>
						<tr><td style="text-align:center"> QR ${Math.ceil(cost_amount)}</td><td style="text-align:center">QR ${Math.ceil(amount)}</td><td style="text-align:center">QR ${Math.ceil(amount_with_overheads)}</td><td style="text-align:center">QR ${Math.ceil(profit_amount)}</td><td style="text-align:center">${Math.ceil(profit_percent)} %</td></tr>
					 </table>`
		...
		// 	frm.set_value("design_amount", 0)
		// 	frm.set_value("design_profit", 0)
		// }
		...
		if (frm.doc.company == 'MEP DIVISION - ELECTRA') {
			...
			// 	frm.set_value("materials_profit", 0)
			// }
			var cost_amount1 = 0
			...
			var table = `<table border='1' style="width:100%">
						...
						<tr><td style="text-align:center"> QR ${Math.ceil(cost_amount1)}</td><td style="text-align:center">QR ${Math.ceil(transfer_amount)}</td><td style="text-align:center">QR ${Math.ceil(transfer_cost_amount)}</td><td style="text-align:center">QR ${Math.ceil(selling_amount)}</td><td style="text-align:center">QR ${Math.ceil(profit_amount)}</td><td style="text-align:center">${Math.ceil(profit_percent)} %</td></tr>
					 </table>`
			...
			var profit_amount = Math.ceil(amount_with_overheads - amount)
			var profit_percent = Math.ceil((amount / amount_with_overheads) * 100)
		...
		// }
		// else {
		// 	frm.set_value("installation_profit", 0)
		// 	frm.set_value("installation_cost", 0)
		// 	frm.set_value("installation_amount", 0)
		// }
		...
		var total_cost = frm.doc.design_cost + frm.doc.materials_cost + frm.doc.finishing_work_cost + frm.doc.accessories_cost + frm.doc.installation_cost + frm.doc.manpower_cost + frm.doc.heavy_equipments_cost + frm.doc.others_cost
		...
		var total_amount_of_profit = total_cost * (frm.doc.total_profit / 100)
		...
		var total_bidding_price = total_cost + total_overheads + frm.doc.business_promotion
		...
		total_business_promotion += frm.doc.business_promotion
		...
		var total_additions = total_business_promotion + total_overheads
		...
		var addnl_rate = total_additions / total_qty
		$.each(frm.doc.design, function (i, d) {
			d.rate_with_overheads = d.unit_price + addnl_rate
			d.amount_with_overheads = d.qty * d.rate_with_overheads
		})
		...
		$.each(frm.doc.manpower, function (i, d) {
			d.rate_with_overheads = d.rate + addnl_rate
			d.amount_with_overheads = d.total_workers * d.rate_with_overheads
		})
